...
int client_count;
struct bufferevent* bevs[MAX_CLIENTS_NUM];
...
static void read_callback(struct bufferevent *bev, void *ctx)
{
	...
	for(int i = 0; i < client_count; i++){
		evbuffer_add(bufferevent_get_output(bevs[i]), received, BUFF_SIZE);
	}
}
		...
		void *ctx)
{
	...
	set_tcp_no_delay(fd);
	bufferevent_setcb(bev, read_callback, NULL, event_cb, NULL);
